...
namespace Tiandao.Plugins
{
	...
	/// </summary>
	/// <remarks>
	///		<para>插件加载器根据一系列策略进行插件加载，可以通过<seealso cref="Tiandao.Plugins.PluginLoader.Plugins"/>或<seealso cref="Tiandao.Plugins.PluginTree.Plugins"/>属性获取加载成功的所有根插件集。</para>
	///		<para>关于插件加载中的相关定义如下：</para>
	///		<list type="table">
	///			<item>
	///				<term>插件根目录</term>
	///				<description>由<seealso cref="Tiandao.Plugins.PluginSetupBase.PluginsDirectory"/>指定的完全限定路径，默认为当前应用程序目录下的plugins文件夹。</description>
	///			</item>
	///			<item>
	///				<term>插件目录</term>
	///				<description>
...
#endif
	{
		...
		/// <remarks>该事件由Load方法激发，在对应的<seealso cref="Tiandao.Plugins.PluginLoader.PluginLoading"/>事件被激发后，该事件不一定总会得到激发，因为加载过程可能出现异常。在<seealso cref="Tiandao.Plugins.PluginLoadedEventArgs"/>事件参数中通过Plugin属性获取到成功加载的插件对象。</remarks>
		...
		/// <remarks>该事件由Unload方法激发，在对应的<seealso cref="Tiandao.Plugins.PluginLoader.PluginUnloading"/>事件被激发后，该事件不一定总会得到激发，因为卸载过程可能出现异常。在<seealso cref="Tiandao.Plugins.PluginUnloadedEventArgs"/>事件参数中通过Plugin属性获取到成功卸载的插件对象。</remarks>
		...
		/// <summary>
		/// 获取加载器的当前加载配置对象。该属性值可通过带参的<seealso cref="Tiandao.Plugins.PluginLoader.Load(PluginLoaderSetup)"/>方法注入。
		/// </summary>
		...
		/// </summary>
		/// <remarks>
		///		<para>在调用该方法前，必须确保当前<seealso cref="Tiandao.Plugins.PluginLoader.Settings"/>属性已被初始化，否则请使用带<seealso cref="Tiandao.Plugins.PluginLoaderSetup"/>类型参数的Load方法重载版本。</para>
		///		<para>通过<seealso cref="Tiandao.Plugins.PluginTree.Loader"/>属性获取的加载器对象默认已经通过<seealso cref="Tiandao.Plugins.PluginContext"/>上下文对象的Settings属性初始化过加载器的<see cref="PluginLoaderSetup"/>配置对象。</para>
		/// </remarks>
		...
		/// <remarks>
		///		<para>使用不同的<see cref="Tiandao.Plugins.PluginLoaderSetup"/>设置项多次加载，会导致最后一次加载覆盖上次加载的插件结构，这有可能会影响您的插件应用对构件或服务的获取路径，从而导致不可预知的结果。</para>
		...
		public void Load(PluginLoaderSetup settings)
		{
			...
			{
				//写入错误日志
				Tiandao.Diagnostics.Logger.Error(ex);
		...
		/// <remarks>
		///		<para>如果指定的插件状态不是已经加载的（即插件对象的Status属性值不等于<seealso cref="Tiandao.Plugins.PluginStatus.Loaded"/>），则不能对其进行卸载。</para>
		/// </remarks>
		...
		private Plugin LoadPluginManifest(string filePath, Plugin parent, PluginLoaderSetup settings)
		{
			...
			if(parent == null)
			{
				if(_plugins.Any(p => string.Equals(p.Name, plugin.Name, StringComparison.OrdinalIgnoreCase)))
				{
					...
					//写入错误日志(原因为插件名称重复)
					Tiandao.Diagnostics.Logger.Error(null, string.Format("The name is '{0}' of plugin was exists. it's path is: '{1}'", plugin.Name, plugin.FilePath));
		...
		private void LoadPluginContent(Plugin plugin, PluginLoaderSetup settings)
		{
			...
			catch(Exception ex)
			{
				...
				//写入错误日志
				Tiandao.Diagnostics.Logger.Error(null, string.Format("The '{0}' plugin file resolve failed.", plugin.FilePath), ex);
			}
		}
