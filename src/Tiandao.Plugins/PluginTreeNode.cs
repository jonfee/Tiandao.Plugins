...
namespace Tiandao.Plugins
{
    public class PluginTreeNode : PluginElement
	{
		...
		/// <remarks>只有当节点类型<see cref="NodeType"/>为构件(<seealso cref="Tiandao.Plugins.PluginTreeNodeType.Builtin"/>)，返回其对应的Builtin的目标值。如果是<see cref="Tiandao.Plugins.PluginTreeNodeType.Custom"/>类型则返回节点值本身；如果是空节点类型则返回空(null)。</remarks>
