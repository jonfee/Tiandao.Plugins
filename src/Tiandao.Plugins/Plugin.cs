...
namespace Tiandao.Plugins
{
...
#endif
	{
		...
		/// </summary>
		/// <remarks>
		///		<para>该属性值为<see cref="Tiandao.Plugins.PluginStatus.Failed"/>时，表示插件在插件的解析或加载过程发生了错误，该状态的插件不会出现在根插件集或对应父插件的子集中。</para>
		///		<para>如果要处理插件在解析或加载过程中发生的异常情况，请使用<seealso cref="Tiandao.Plugins.PluginLoader.Failure"/>事件。</para>
		/// </remarks>
		...
		/// 获取当前插件的父插件对象。
		/// </summary>
		/// <remarks>关于父插件定义和父插件的搜索策略，请参考<seealso cref="Tiandao.Plugins.PluginLoader"/>类的帮助。</remarks>
		...
		/// <exception cref="System.InvalidOperationException">当插件树尚未初始化，即<seealso cref="Tiandao.Plugins.PluginTree.Status"/>属性值为<seealso cref="Tiandao.Plugins.PluginTreeStatus.None"/>时。</exception>
		...
		/// <returns>如果指定的<seealso cref="Tiandao.Plugins.Plugin"/>等于当前的<seealso cref="Tiandao.Plugins.Plugin"/>，则为 true；否则为 false。</returns>
		/// <remarks>该相等性是按照插件的<see cref="Tiandao.Plugins.Plugin.FilePath"/>文件名进行不区分大小写比较。</remarks>
