	...
	public class ServicesParser : Parser
	{
		...
		private T GetValue<T>(ParserContext context, Func<IServiceProvider, object, bool, T> thunk) where T : class
		{
			...
			if(serviceName.StartsWith("~") || serviceName.StartsWith("*"))
			{
				...
				if(typeof(Tiandao.Services.IServiceProvider).IsAssignableFrom(context.MemberType) || typeof(System.IServiceProvider).IsAssignableFrom(context.MemberType))
					return thunk(provider, null, false);
		...
		private object ResolveService(string serviceName, Func<object> serviceThunk)
		{
			...
			return Tiandao.Common.Converter.GetValue(serviceThunk(), members);
		}
